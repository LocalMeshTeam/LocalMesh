...
function friendlyError(error: string): string {
  ...
  if (clean.includes("File transfer not found")) return "This file is no longer available on this device.";
  if (clean.includes("ENOENT") || clean.includes("no such file or directory")) return "This file is no longer available on this device.";
  return clean;
}
...
function App() {
  ...
  const downloadFile = async (transferId: string) => {
    try {
      const saved = await window.localmesh.downloadReceivedFile(transferId);
      if (saved) setError("");
    } catch (reason) {
      const message = reason instanceof Error ? reason.message : String(reason);
      setError(message.replace(/^Error invoking remote method '[^']+': Error: /, ""));
