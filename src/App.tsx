  ...
  return <main className="shell">
    ...
    <div className="layout">
      <aside className="sidebar">
        ...
        <section><h2>Conversations</h2>{conversations.length === 0 && <p className="muted">No conversations yet.</p>}{conversations.map((conversation) => <button className={`conversation ${selected?.conversation_id === conversation.conversation_id ? "selected" : ""}`} key={conversation.conversation_id} onClick={() => setSelected(conversation)}><span className="conversation-name">{peerLabel(conversation.peer_id)}</span><small>{peers.some((peer) => peer.device_id === conversation.peer_id) ? "Online" : "Offline"} · Device {shortDeviceId(conversation.peer_id)}</small></button>)}</section>
      </aside>
